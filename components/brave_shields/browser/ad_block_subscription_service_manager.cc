...
void AdBlockSubscriptionServiceManager::Init(
  ...
  filters_combinator_ = std::make_unique<AdBlockCombinatorFiltersProvider>();
  subscription_source_observer_ =
      std::make_unique<AdBlockService::SourceProviderObserver>(
          subscription_engine_->AsWeakPtr(), filters_combinator_.get(),
...
void AdBlockSubscriptionServiceManager::CreateSubscription(
  ...
  {
    base::AutoLock lock(subscription_services_lock_);
    filters_combinator_->CombinateProvider(subscription_filters_provider.get());
...
AdBlockSubscriptionServiceManager::GetSubscriptions() {
  ...
  for (const auto subscription : subscriptions_) {
    auto info = GetInfo(subscriptions_, GURL(subscription.first));
    DCHECK(info);
...
absl::optional<SubscriptionInfo> AdBlockSubscriptionServiceManager::GetInfo(
    ...
    const GURL& sub_url) {
  auto* list_subscription_dict = subscriptions.FindDict(sub_url.spec());
  if (!list_subscription_dict)
    return absl::nullopt;
  ...
  for (const auto it : subscriptions_) {
    ...
    if (list_subscription_dict) {
      ...
      if (info.enabled) {
        ...
        filters_combinator_->CombinateProvider(
