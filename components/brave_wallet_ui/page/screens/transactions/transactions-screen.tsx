      ...
      <Row>
        ...
        <NetworkFilterSelector
          onSelectNetwork={(n) => {
            history.push(
              updatePageParams({
                address: foundAccountFromParam?.address || undefined,
                chainId,
                transactionId
              })
            )
...
function updatePageParams ({
  ...
  return WalletRoutes.Transactions
    ...
    .replace('/:chainId?', address && chainId ? `/${chainId}` : '')
    .replace('/:transactionId?', chainId && transactionId ? `/${transactionId}` : '')
}
