...
- (instancetype)initWithUserAgent:(NSString*)userAgent
  ...
  if ((self = [super init])) {
    ...
    _otr_browserList = BrowserListFactory::GetForBrowserState(otrChromeBrowserState);
...
- (void)dealloc {
  ...
  _otr_browserList = BrowserListFactory::GetForBrowserState(_otr_browser->GetBrowserState());
  [_otr_browser->GetCommandDispatcher() prepareForShutdown];
  _otr_browserList->RemoveBrowser(_otr_browser.get());
  _otr_browser->GetWebStateList()->CloseAllWebStates(WebStateList::CLOSE_NO_FLAGS);
  _otr_browser.reset();
