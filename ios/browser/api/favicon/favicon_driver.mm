...
#import "brave/ios/browser/web/web_state/brave_web_state+private.h"
...
class FaviconDriverObserver final : public favicon::FaviconDriverObserver {
  public:
    FaviconDriverObserver(base::RepeatingCallback<void(const GURL& icon_url, const gfx::Image& image)> callback);
  ...
  private:
    base::RepeatingCallback<void(const GURL& icon_url, const gfx::Image& image)> callback_;
};

FaviconDriverObserver::FaviconDriverObserver(base::RepeatingCallback<void(const GURL& icon_url, const gfx::Image& image)> callback) : callback_(callback) {
  
}

FaviconDriverObserver::~FaviconDriverObserver() = default;

void FaviconDriverObserver::OnFaviconUpdated(favicon::FaviconDriver* favicon_driver,
                                             NotificationIconType notification_icon_type,
                                             const GURL& icon_url,
                                             bool icon_url_changed,
...
@property (nonatomic, weak, readonly) BraveWebState* webState;
@end

@implementation BraveFaviconDriver
- (instancetype)initWithWebState:(BraveWebState*)webState {
  if ((self = [super init])) {
    _webState = webState;
...
- (void)webView:(WKWebView*)webView
  ...
  if (!urls.empty()) {
    driver_observer_ = std::make_unique<FaviconDriverObserver>(base::BindRepeating(^(const GURL& icon_url, const gfx::Image& icon){
    ...
    static_cast<web::WebStateImpl*>(web_state)->OnNavigationStarted(context.get());
    static_cast<web::WebStateImpl*>(web_state)->OnNavigationFinished(context.get());
