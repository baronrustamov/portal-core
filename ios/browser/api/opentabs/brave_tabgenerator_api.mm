...
@property(nonatomic, strong) BraveWebState* web_state;
@end
...
- (instancetype)initWithBrowser:(Browser*)browser isOffTheRecord:(bool)isOffTheRecord {
  if ((self = [super init])) {
    _web_state = [[BraveWebState alloc] initWithBrowser:browser isOffTheRecord:isOffTheRecord];
  }
  return self;
}

- (BraveWebState*)webState {
  return _web_state;
}
...
- (BraveSyncTab*)createBraveSyncTab:(bool)isOffTheRecord {
  return [[BraveSyncTab alloc] initWithBrowser:browser_ isOffTheRecord:isOffTheRecord];
}
