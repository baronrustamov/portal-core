...
#pragma mark BraveNativeWebState

BraveNativeWebState::BraveNativeWebState(Browser* browser, bool off_the_record)
    : browser_(browser),
...
BraveNativeWebState::~BraveNativeWebState() {
...
void BraveNativeWebState::SetTitle(const std::u16string& title) {
  DCHECK(web_state_);
...
void BraveNativeWebState::SetURL(const GURL& url) {
  DCHECK(web_state_);
  ...
  if (web_state_) {
    ...
    navigation_manager->AddPendingItem(
        ...
        web::NavigationInitiationType::BROWSER_INITIATED, /*is_post_navigation=*/false,
...
base::WeakPtr<web::WebState> BraveNativeWebState::GetWeakWebState() {
  DCHECK(web_state_);
  return web_state_->GetWeakPtr();
}

BraveNativeWebState::Observer::Observer(BraveNativeWebState* state) : native_state_(state) {
  DCHECK(native_state_);
  DCHECK(native_state_->web_state_);
  native_state_->web_state_->AddObserver(this);
}

BraveNativeWebState::Observer::~Observer() {
...
void BraveNativeWebState::Observer::WebStateDestroyed(web::WebState* web_state) {
